...
In this project, I tried to create a grocery list generator using Python and Google sheets as a database. The user can choose from a selection of recipes, and the program will generate a grocery list with all the necessary ingredients based on the user’s choices. The program also gives the user the option to check their stock before generating the list, making the process of figuring out what ingredients you should buy, and what you already have in inventory, a lot easier.
...
- I want to create a program that is easy to navigate.
    ...
    - How was this achieved?
        - By using numbers as the option choices. The user can choose between 1 and 2 for YES or NO questions, and between 1 and 4 for dish types and dish recipe questions.
...
- I want the program to run in a loop asking the user if they want to add another meal.
    - Was this achieved?
        - Yes
    - How was this achieved?
        - After picking a meal, the program will ask the user if they want to add another meal, 1 for YES or 2 for NO. If the user types 1, it will go back to the dish type selection screen. If the user types 2, it will finish the recipe selection phase.
- I want the program to display the selected recipes on-screen when asking the user to add another meal.
    - Was this achieved?
        - Yes
    - How was this achieved?
        - By placing a print statement that displays the recipe list inside the while True loop in the another_meal() function that handles this part of the code.
- I want the program to give the user the option to have the complete grocery list, or to have an updated list with the 'ingredients in stock' removed from it.
    - Was this achieved?
        - Yes
    - How was this achieved?
        - After selecting meals, the program will ask if the user wants to check their stock before generating a grocery list, 1 for YES or 2 for NO. If 1, is selected the program will compare the complete grocery list against the ingredients in stock and generate the updated list. If 2 is selected a complete grocery list with no adjustments will be generated.
...
- Initially, I had the idea to create a program that would have the stock database updated after an ingredient is used for a recipe. Due to the time limit and security parameters, I decided to keep the database read-only.
...
- The instruction page explains to the user how the program works and then asks the user if they want to pick a meal or to see the stock.
...
- When the user is done adding meals, the program will ask if they want to use the ingredients from their stock.

<img src="images/using_stock.png" alt="Screenshot of ask to use stock page">

### Grocery list using stock

- If the user chooses to use the ingredients from their stock, the program will remove the ingredients in stock before generating the grocery list.

- A 'thank you' message is displayed, and the option to rerun the program is given.

- If the user decides to rerun the program, it will display the 'welcome' page again.
...
- If the user decides not to use the ingredients from their stock, a grocery list with all the ingredients necessary for the recipes is generated.
...
- If the user decides to see their stock, a table is displayed showing all the ingredients in thier stock.
...
- If two is pressed, the 'welcome' page will load.
...
<img src="images/seeing_stock.png" alt="meal or stock manual testings"> 
...
- As you can see, when check stock was selected, the ingredient broccoli was removed from the list because it was already in stock.
...
There were many steps involved in deploying this project to Heroku:
...
- I would need to commit and push this to Github.
...
6. My program included sensitive data used to access Google sheets that isn't in the github repository because it has been deliberately protected in the gitnore.file. 
...
7. I added the buildpacks needed by clicking on the buildpack button.
 ...
 - Then repeated the same process but selected nodejs this time, making sure it was done in that order with Python at the top and nodejs under.
...
11. I then scrolled down to where there were two options.
...
12. When all the code is received from github, there is a view button that has a link to the running app, I clicked this to make sure everything was running as expected.

Go back to [Table of contents](#table-of-contents)

## Credits 

- [Lucid chart](https://www.lucidchart.com/pages/) - was used to build my flowchart. 
...
- My mentor Marcel helped me through this project.
